...
describe("preflight", () => {
  ...
  it("blocks publishing when generated first-build images are saved but not attached", () => {
    const result = preflight({
      ...healthy(),
      firstBuildImagery: {
        ...
        generated: 3,
        attached: 1,
        message: "Generated 3 starter website images; 1 reached the visitor-facing draft.",
      },
    });
    expect(result.publishSafe).toBe(false);
    expect(result.blockers.map((b) => b.key)).toContain("first-build-images");
  });

  it("passes starter imagery when every generated image reaches the draft", () => {
    const result = preflight({
      ...healthy(),
      firstBuildImagery: {
        generatedStatus: "generated",
        generated: 2,
        attached: 2,
        message: "Generated 2 starter website images; 2 reached the visitor-facing draft.",
      },
    });
    expect(result.blockers.map((b) => b.key)).not.toContain("first-build-images");
    expect(result.checks.find((c) => c.key === "first-build-images")?.status).toBe("pass");
  });

  it("keeps publishing gated until fresh browser visual evidence is ready", () => {
    const result = preflight({
      ...healthy(),
      visualQa: {
        content: "PASS",
        ...
        ready: false,
        reason: "No fresh 320-1440px visual pass has been recorded for this draft.",
      },
    });
    expect(result.publishSafe).toBe(false);
    expect(result.blockers.map((b) => b.key)).toContain("browser-visual-qa");
  });

  it("accepts fresh browser visual evidence when it is explicitly ready", () => {
    const result = preflight({
      ...healthy(),
      visualQa: {
        content: "PASS",
    ...
    expect(result.blockers.map((b) => b.key)).not.toContain("browser-visual-qa");
    expect(result.checks.find((c) => c.key === "browser-visual-qa")?.status).toBe("pass");
  });
});
